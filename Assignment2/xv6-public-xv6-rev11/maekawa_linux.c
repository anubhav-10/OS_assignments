		...
		if (fork() == 0) {
			...
			for(i = 0; i < size; i++) {
				...
				for(;;) {
					...
					// printf("pid %d type %c grant id %d\n",k, ret.type, ret.id);
					// for(int z=0;z<10;z++)
					// 	printf("pid: %d request: %c %d\n", k, ret.type, ret.id);
					// sleep(1);
					if (ret.type == 'R'){
						// printf("pid %d received release from %d\n", k, ret.id);
						break;
					}
					else if (ret.type == 'G') {
						// printf("pid %d received grant from %d\n", k, ret.id);
						// int flag = 1;
						grants++;
						// printf("pid %d grants %d\n", k, grants);
						if (grants == P) {
							int time = 0;
							if(k >= (P1 + P2)) time = 2;
							// printf("%d Entered critical \n", k);
							// printf("%d Exited critical \n", k);
							// if(k >= P1)
